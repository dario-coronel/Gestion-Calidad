...
class DashboardQrKpiTests(TestCase):
    ...
    def test_dashboard_muestra_kpis_y_tendencia_qr(self):
        ...
        self._crear_om(fecha=hoy - timedelta(days=2), estado=EstadoOM.IMPLEMENTADA)
        ...
        self.assertEqual(response.context['kpi_total_om'], 3)
        self.assertEqual(response.context['kpi_om_abiertas'], 2)
        self.assertEqual(response.context['kpi_om_implementadas'], 1)
